...
//                v.getContext().startActivity(intent);
    ...
    public void  onBindViewHolder(ViewHolder holder,int position){
        ...
        if(grade.getEvaluate1()==0){
            //holder.evaluate1.setTextColor(Color.RED);
            holder.evaluate1.getPaint().setFlags(Paint. STRIKE_THRU_TEXT_FLAG ); //中间横线（删除线）;
        }
        else{
            holder.evaluate1.setTextColor(Color.RED);
        }
        if(grade.getEvaluate2()==0){
            //holder.evaluate2.setTextColor(Color.RED);
            holder.evaluate2.getPaint().setFlags(Paint. STRIKE_THRU_TEXT_FLAG ); //中间横线（删除线）
        }
        else{
            holder.evaluate2.setTextColor(Color.RED);
        }
        if(grade.getEvaluate3()==0){
           // holder.evaluate3.setTextColor(Color.RED);
            holder.evaluate3.getPaint().setFlags(Paint. STRIKE_THRU_TEXT_FLAG ); //中间横线（删除线）
        }else{
            holder.evaluate3.setTextColor(Color.RED);
        }
